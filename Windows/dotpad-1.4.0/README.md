...
* callback function
  ...
  ### DOT_PAD_SDK_ERROR DOT_PAD_REGISTER_MULTI_KEY_CALLBACK(void(CALLBACK* cb)(int), int interval);
* the function that registers a callback function to be called by multi key input
    ```
    return
        DOT_PAD_SDK_ERROR
    parameter
        1st: function pointer
        2nd: the number of multikey interval
    ```
* callback function
    ```
    return
    ...
    parameter
        callback function pointer
        - pressed key information(e.g. 0x04/0x80/0x40/0x20/0x10/0x02 from the left on 320 Dot Pad)
    ```  
