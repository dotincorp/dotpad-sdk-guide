...
extern "C"
{
	...
	// DotPadKey: onKeyDown/onKeyUp 전용 Key (단일 키 눌림/뗌만 감지, 조합키는 DOT_KEY_CODE에서만 처리)
	typedef enum {
		DOT_PAD_KEY_FUNCTION1 = 0,
		DOT_PAD_KEY_FUNCTION2,
		DOT_PAD_KEY_FUNCTION3,
		DOT_PAD_KEY_FUNCTION4,
		DOT_PAD_KEY_PANNING_LEFT,
		DOT_PAD_KEY_PANNING_RIGHT
	} DOT_PAD_KEY;

	// DotPadLanguage: 점역(Braille translation) 언어 종류.
	// DOT_PAD_BRAILLE_DISPLAY / DOT_PAD_TRANSLATE_TEXT / DOT_PAD_SET_LANGUAGE의 language 인자에 사용.
	typedef enum {
		DOT_PAD_LANGUAGE_NONE = 0,
		DOT_PAD_LANGUAGE_ARABIC,
		DOT_PAD_LANGUAGE_CHINESE_TRADITIONAL,
		DOT_PAD_LANGUAGE_CHINESE_SIMPLIFIED,
		DOT_PAD_LANGUAGE_DUTCH,
		DOT_PAD_LANGUAGE_ENGLISH,
		DOT_PAD_LANGUAGE_FRENCH,
		DOT_PAD_LANGUAGE_GERMAN,
		DOT_PAD_LANGUAGE_ITALIAN,
		DOT_PAD_LANGUAGE_JAPANESE,
		DOT_PAD_LANGUAGE_KOREAN,
		DOT_PAD_LANGUAGE_RUSSIAN,
		DOT_PAD_LANGUAGE_SPANISH,
		DOT_PAD_LANGUAGE_VIETNAMESE,
		DOT_PAD_LANGUAGE_BULGARIAN,
		DOT_PAD_LANGUAGE_PORTUGUESE,
		DOT_PAD_LANGUAGE_CZECH,
		DOT_PAD_LANGUAGE_POLISH,
		DOT_PAD_LANGUAGE_NORWEGIAN,
		DOT_PAD_LANGUAGE_KAZAKH,
		DOT_PAD_LANGUAGE_DANISH,
		DOT_PAD_LANGUAGE_GREEK,
		DOT_PAD_LANGUAGE_SWEDISH,
		DOT_PAD_LANGUAGE_FINNISH,
		DOT_PAD_LANGUAGE_THAI,
		DOT_PAD_LANGUAGE_UZBEK
	} DOT_PAD_LANGUAGE;
...
#define DOTPADSDK_API __declspec(dllimport)
	...
	namespace DOT_PAD_SDK_API {
		...
		// ========== 점역 (기기 연결 불필요) ==========
		// 텍스트를 점자 데이터로 변환만 수행 (기기로 전송하지 않음). cellCount 배수로 0 패딩되어 반환됨.
		DOTPADSDK_API bool DOT_PAD_TRANSLATE_TEXT(const wchar_t* strInput, int cellCount, void(CALLBACK* callback)(const uint8_t* translatedData, size_t dataSize));
		...
		// 키 다운 콜백 (단일 키가 눌리는 순간, dotKeyBinary: 해당 키 그룹의 비트마스크 2진 문자열)
		DOTPADSDK_API void DOT_PAD_REGISTER_KEY_DOWN_CALLBACK(void(CALLBACK* cb)(void* deviceHandle, DOT_PAD_KEY key, const char* dotKeyBinary));

		// 키 업 콜백 (단일 키가 떼어지는 순간, dotKeyBinary: 해당 키 그룹의 비트마스크 2진 문자열)
		DOTPADSDK_API void DOT_PAD_REGISTER_KEY_UP_CALLBACK(void(CALLBACK* cb)(void* deviceHandle, DOT_PAD_KEY key, const char* dotKeyBinary));

		// ========== 진동 ==========
		// 진동 요청 (deviceHandle이 nullptr이면 연결된 모든 기기에 요청, onMs: 진동 ON 시간(ms), offMs: 멈춤 시간(ms), repeatCount: 펄스 수(1~5))
		DOTPADSDK_API bool DOT_PAD_REQUEST_VIBRATOR(void* deviceHandle, int onMs, int offMs, int repeatCount);
	}
	...
	namespace DOT_PAD_SDK_API {
		...
		// 점역 (기기 연결 불필요)
		typedef bool(*DOT_PAD_TRANSLATE_TEXT_FUNC)(const wchar_t* strInput, int cellCount, void(CALLBACK* callback)(const uint8_t* translatedData, size_t dataSize));
		...
		typedef void(*DOT_PAD_REGISTER_KEY_DOWN_CALLBACK_FUNC)(void(CALLBACK* cb)(void* deviceHandle, DOT_PAD_KEY key, const char* dotKeyBinary));
		typedef void(*DOT_PAD_REGISTER_KEY_UP_CALLBACK_FUNC)(void(CALLBACK* cb)(void* deviceHandle, DOT_PAD_KEY key, const char* dotKeyBinary));

		// 진동
		typedef bool(*DOT_PAD_REQUEST_VIBRATOR_FUNC)(void* deviceHandle, int onMs, int offMs, int repeatCount);
	}
#endif
