...
class DotPadSendModule {
    ...
    // setTimeout은 백그라운드 탭에서 최소 1000ms로 스로틀됨(CET-83) → 탭이 숨겨진 동안만
    // MessageChannel(지연 없음)로 대체한다. 포그라운드에서까지 MessageChannel을 쓰면 ACK
    // 대기 중에도 매 tick 즉시 재스케줄되어 브라우저 태스크 큐를 계속 점유하게 되고, 그 사이
    // BLE 알림(키 입력 등) 처리가 밀리는 현상이 실측됐다 — 그래서 포그라운드에서는 약간의
    // 실제 지연(setTimeout)을 둬서 다른 이벤트가 끼어들 틈을 준다.
    #scheduleNext(fn) {
...
// liblouis(wasm) 정적 파일(liblouis.js/.wasm/.data) 위치. 배포 시 세 파일을 반드시 함께 옮겨야
// 한다(.data 안에 tables 전체가 미리 패키징되어 있음). 번들 앱에서는 항상 setAssetBaseUrl()로
// 실제 정적 서빙 경로를 지정해서 써야 한다(README 참고) — 아래 기본값은 그 호출 전에 실수로
// loadModule()이 불려도 죽지 않게 하는 최후의 폴백일 뿐, 번들된 앱에서는 맞는 경로가 아니다.
//
// new URL(literal, import.meta.url) 형태를 webpack5/Turbopack 등 번들러가 "모듈 상대 정적
// 에셋 참조"로 해석해 그 경로를 소스 트리에서 직접 찾으려 시도하는 관례가 있다. Next.js
// 16(Turbopack)에서 실측한 동작:
//  - new URL(...) 인자 목록 안에 "./lib/" 같은 상대경로 문자열 리터럴이 (변수로 감싸거나 함수
//    인자로 한 단계 우회해도) 텍스트로 나타나면 무조건 정적 에셋으로 취급해 "Module not found:
//    Can't resolve './lib/'"로 빌드가 깨진다 — 실제로 그 경로에 파일을 다 갖춘 lib/ 폴더를
//    만들어놔도 동일하게 깨진다(디렉터리 자체가 이 관례가 기대하는 대상이 아닌 듯하다).
//    webpackIgnore/turbopackIgnore 매직 주석은 import()에만 적용되고 이 패턴에는 안 먹힌다.
//  - 두 번째 인자가 import.meta.url로 추적되는 식(리터럴이든 변수든)이면, 첫 인자가 진짜
//    동적(함수 매개변수)이어도 "Module not found: Can't resolve <dynamic>"으로 깨진다.
//    두 번째 인자를 import.meta.url과 무관한 식(예: window.location.href 삼항)으로 바꾸면
//    이 경우는 피해진다.
// 따라서 기본값 계산은 (1) 문자코드로 조립해 "./lib/" 리터럴이 소스에 텍스트로 존재하지
// 않게 하고, (2) 두 번째 인자도 import.meta.url을 직접 참조하지 않게 만든다.
const moduleUrl = import.meta.url;
let assetBaseUrl = null;
let modulePromise = null;

function resolveDefaultBaseUrl() {
    const dir = String.fromCharCode(46, 47, 108, 105, 98, 47); // "./lib/"
    return new URL(dir, typeof window !== "undefined" ? window.location.href : moduleUrl);
}

/**
 * liblouis 정적 파일을 다른 경로에 배포했을 때 사용. loadModule()이 처음 호출되기 전에만 유효하다.
 * 번들 앱(webpack/Turbopack/Vite 등)에서는 기본 경로가 맞지 않으므로 항상 호출해야 한다.
 * @param {string | URL} url - liblouis.js/.wasm/.data 세 파일이 위치한 디렉토리(끝에 "/" 필요). 절대
 *   URL 권장(README 예시처럼 `${window.location.origin}/...`); 상대 경로는 현재 페이지 기준으로 해석된다.
 */
function setAssetBaseUrl(url) {
    assetBaseUrl = new URL(url, typeof window !== "undefined" ? window.location.href : moduleUrl);
}

// liblouis(wasm) 엔진 로드 — 최초 1회만 초기화하고 이후 호출은 같은 Promise를 재사용한다.
function loadModule() {
    if (!assetBaseUrl) assetBaseUrl = resolveDefaultBaseUrl();
    if (!modulePromise) {
        modulePromise = import(/* @vite-ignore */ /* webpackIgnore: true */ /* turbopackIgnore: true */ new URL("liblouis.js", assetBaseUrl).href)
