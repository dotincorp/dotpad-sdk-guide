...
> **v3.0.3 fixes (no API changes):**
> - Fixed a Next.js (Turbopack) / webpack 5 build failure (`Module not found: Can't resolve './lib/'`) that could occur when bundling this SDK, caused by the bundler statically detecting the liblouis default asset-path pattern in the SDK's own source — regardless of whether your app ever relies on that default path.
> - Fixed a bug where a key event (or other status notification) arriving in the same BLE packet batch as a display acknowledgment could be silently dropped instead of processed.
> - Fixed a scheduling issue where the display-output retry loop could, while the browser tab is in the foreground, monopolize the main thread and delay delivery of other events (including key presses) until it yielded.

---

## SDK Files

Download and unzip [`download/web-sdk-3.0.3.zip`](download/web-sdk-3.0.3.zip):

| File | Description |
|---|---|
| `DotPadSDK-3.0.3.js` | SDK library (ES module) |
| `DotPadSDK-3.0.3.d.ts` | TypeScript type definitions |
...
Unzip `download/web-sdk-3.0.3.zip`, then copy `DotPadSDK-3.0.3.js` and `DotPadSDK-3.0.3.d.ts` into your project and import directly:

```ts
import { DotPadScanner, DotPadSDK, DotDevice, DataCodes, KeyCodes, DotPadKey, DisplayMode } from "./DotPadSDK-3.0.3";
```
...
```ts
import { LiblouisManager } from "./DotPadSDK-3.0.3";

LiblouisManager.setAssetBaseUrl(`${window.location.origin}/liblouis/`);
```

If you don't call `setAssetBaseUrl`, the SDK falls back to `lib/` resolved relative to the current page URL — this only reliably works for simple `<script type="module">` setups where the SDK file, the HTML page, and the `lib/` folder all share the same base path. For any bundled app (webpack, Vite, Next.js/Turbopack, etc.), always call `setAssetBaseUrl` before the first translation call, as shown above.

As of v3.0.3, this fallback path no longer triggers a Next.js/Turbopack build failure — previously, even apps that always called `setAssetBaseUrl` (and so never relied on the fallback at runtime) could still hit `Module not found: Can't resolve './lib/'` at build time, because the bundler statically scanned the SDK's own source for that path. This is now fixed.
...
```ts
import { BrailleLanguage, GradeOption, NumberOfBraillePerLine, LiblouisManager } from "./DotPadSDK-3.0.3";
...
3.0.3/
├── download/
│   └── web-sdk-3.0.3.zip      ← unzip for DotPadSDK-3.0.3.js/.d.ts + lib/ (see SDK Files above)
...
> **v3.0.3:** internal fixes only (see the notice at the top of this document) — no Demo App or public API changes.
>
