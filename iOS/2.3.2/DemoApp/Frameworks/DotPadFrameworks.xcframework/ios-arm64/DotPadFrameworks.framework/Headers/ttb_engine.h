...
*/
int BrailleTranslator(uint16_t* input, uint8_t* output, int appOpt[NUM_USER_OPT]);
int MultilineBrailleTranslator(char* input, char* output, int appOpt[NUM_USER_OPT]);
int MultilineBrailleTranslatorWithOptionLength(char* input, char* output, int appOpt[NUM_USER_OPT], int appOptLen);
int ImageBrailleTranslator(char* input, char* output, int appOpt[NUM_USER_OPT]);

int changeGraphicDisplayText(char* input, char* output);
int changeGraphicDisplayTextLine(char* input, char* output, int columsNum, int paddingNum);

void TableBrailleData(char* tableData, int numRows, int numCols, int currentSentenceIndex, char* output, int appOpt[NUM_USER_OPT]);
void TableDataMachingData(char* tableJson, int numRows, int numCols, char* output);
int wrap_braille_hex(const char* input, int lineWidth, char* output, int* outputLength);
